...
export const CheckoutPanel: React.FC<Props> = ({
  ...
  gstCalc, grandTotal, isGSTBill, paymentMethod,
    ...
    <div className="w-80 bg-checkout text-checkout-foreground flex flex-col border-l">
      ...
      <div className="p-3 space-y-2 border-b border-checkout-foreground/10">
        ...
        {isGSTBill && (
          <input
            value={customerGST}
            onChange={e => onCustomerGSTChange(e.target.value)}
      ...
      <div className="flex-1 p-3 space-y-2 text-sm pos-scrollable">
        <div className="flex justify-between">
          <span className="text-checkout-foreground/70">Items</span>
          <span>{items.length}</span>
        </div>
        <div className="flex justify-between">
          <span className="text-checkout-foreground/70">Subtotal</span>
          <span className="price-text">{fmt(subtotal)}</span>
        </div>
        {itemDiscountTotal > 0 && (
          ...
          <div className="pt-2 border-t border-checkout-foreground/10">
            ...
            <div className="flex gap-1">
              <input
                ...
                className="flex-1 h-8 px-2 rounded bg-checkout-foreground/10 text-checkout-foreground text-sm focus:outline-none"
                placeholder="0"
              />
              <button
            ...
            {billDiscountAmount > 0 && (
              <div className="flex justify-between mt-1 text-warning">
                <span className="text-xs">Bill Discount</span>
        ...
        {isGSTBill && (
          <div className="pt-2 border-t border-checkout-foreground/10 space-y-1">
            <div className="flex justify-between text-xs">
              <span className="text-checkout-foreground/50">Taxable Amount</span>
              <span>{fmt(gstCalc.taxableAmount)}</span>
            </div>
            <div className="flex justify-between text-xs">
              <span className="text-checkout-foreground/50">CGST</span>
              <span>{fmt(gstCalc.cgst)}</span>
            </div>
            <div className="flex justify-between text-xs">
              <span className="text-checkout-foreground/50">SGST</span>
              <span>{fmt(gstCalc.sgst)}</span>
            </div>
          </div>
        )}
      </div>
            ...
            <button
              ...
              className={`flex flex-col items-center gap-1 py-2 rounded-md text-xs transition-all ${
                paymentMethod === m.key
                  ...
                  : 'bg-checkout-foreground/10 text-checkout-foreground/70 hover:bg-checkout-foreground/20'
              }`}
            >
