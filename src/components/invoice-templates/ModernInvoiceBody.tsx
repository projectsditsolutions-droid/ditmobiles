...
export const ModernInvoiceBody: React.FC<Props> = ({
  ...
  <div style={{ fontFamily: 'Inter, Arial, sans-serif', fontSize: '12px', color: '#111', background: '#fff', padding: '0' }}>
        ...
        {invoice.items.map((item, idx) => {
          ...
          return (
            <tr key={item.id} style={{ borderBottom: '1px solid #e5e7eb', background: idx % 2 === 0 ? '#fafafa' : '#fff' }}>
              ...
              <td style={{ padding: '6px', verticalAlign: 'top', fontFamily: 'monospace', fontSize: '10px' }}>{item.product.hsn_code || '—'}</td>
              <td style={{ padding: '6px', verticalAlign: 'top', fontFamily: 'monospace', fontSize: '10px' }}>{item.imei || '—'}</td>
              ...
              {invoice.is_gst_bill && gst && (
                <>
                  ...
                  <td style={{ padding: '6px', textAlign: 'right', verticalAlign: 'top', color: '#6b7280', fontSize: '10px' }}>
    ...
    <div style={{ display: 'flex', justifyContent: 'flex-end', marginBottom: '14px' }}>
      <div style={{ width: '270px', background: '#f5f3ff', borderRadius: '8px', padding: '12px', fontSize: '11px' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', borderTop: '3px solid #4338ca', marginTop: '6px', paddingTop: '6px', fontWeight: 900, fontSize: '16px', color: '#4338ca' }}>
