  ...
  <div style={{ fontFamily: 'Arial, sans-serif', fontSize: '10px', color: '#111', background: '#fff', padding: '0', maxWidth: '380px', margin: '0 auto' }}>
    ...
    <div style={{ borderTop: '1px solid #ccc', borderBottom: '1px solid #ccc', marginBottom: '8px' }}>
      ...
      {invoice.items.map((item, idx) => (
        <div key={item.id} style={{ display: 'flex', justifyContent: 'space-between', padding: '3px 0', borderTop: '1px dotted #e5e7eb', fontSize: '9px' }}>
          <div style={{ flex: 1 }}>
    ...
    <div style={{ fontSize: '9px', marginBottom: '8px' }}>
      ...
      {invoice.is_gst_bill && (
        <div style={{ display: 'flex', justifyContent: 'space-between', color: '#888' }}>
          <span>GST (C:{invoice.cgst.toFixed(0)} + S:{invoice.sgst.toFixed(0)})</span>
          <span>₹{(invoice.cgst + invoice.sgst).toLocaleString('en-IN')}</span>
        </div>
      )}
